...
if __name__ == "__main__":
    ...
    # 创建一个全是无信号的train_x
    train_x = np.array([[[0, -110] for i in range(LENGTH)] for j in range(len(train_waps_rssis))])
    ...
    # create an array full of no signal WI-FI for later use
    # 创建一个全是无信号的val_x
    val_x = np.array([[[0, -110] for i in range(LENGTH)] for j in range(len(val_waps_rssis))])
    ...
    # 保存模型
    model.save(f"../model_1d2c-{LENGTH}")
